...
g3_simplified <- ggplot(df_webs, aes(x = log10(dispersion),
  ...
  geom_abline(slope = -0.42, intercept = 0) + 
...
m2 <- lm( I (robustness-robustness_hat)  ~ -1 +  I(log10(dispersion)), 
...
g_test <- ggplot(test_df, aes(x = new_f, 
 ...
 theme(axis.text = element_text(size = axis_text_size),
  ...
  geom_point(df_webs, mapping = aes(x = fragility,
                     y = robustness,
                     color = log10(dispersion),
                     group = NULL)) + 
  scale_color_viridis_c() 
...
ggsave("R-by-f-with-d.png")

```

we can then use this relationship to adjust the predicted robustness for a given fragility by accounting for its dispersion

```{r adust-for-dispersion}

# calculate the correction
crx <- (cc[1] + cc[2] * log10(df_webs$dispersion))


g8 <-  ggplot(df_webs, aes(x = fragility,
                     y = robustness - crx,
                     color = log10(dispersion))) + 
  geom_point() + 
  scale_color_viridis_c() + 
...
## Jeff's new figures 2020/3/2

```{r}

df_webs <- mutate(fstar = fragility + 0.42 * log(dispersion))         

jf1 <- ggplot(data = df_webs, 
              mapping = aes(x = fstar, 
                            y = robustness,
                            color = log10(dispersion))) + 
  geom_point()
