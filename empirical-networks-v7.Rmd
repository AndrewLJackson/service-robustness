...
df_webs <- df_webs %>%
  mutate(fstar = pmax(fragility - coef(mRf) * log10(dispersion), runif(n(), 0, 0.1)))
...
# generate the phenomenological curve y = exp(-fstar - fstar^2)
fit_df_fstar <- data.frame(new_fstar = seq(from = 0, to = 2.5, 
                                           length.out = 100)) %>%
  mutate(est_r = exp(-new_fstar - new_fstar^2))
...
g6 <- ggplot(df_webs, aes(x = fstar, 
  ...
  geom_line(data = fit_df_fstar, 
            mapping = aes(x = new_fstar, 
                          y = est_r), 
...
df_webs <- df_webs %>% mutate(resid_simple = robustness - 1 + fstar,
                              resid_fstar = robustness - exp(-fstar - fstar^2))
...
```{r}
df_webs %>% arrange(fstar, desc(S), resid_fstar, robustness) %>% select(file, S, N, resid_fstar, robustness, dispersion, fstar)
```
