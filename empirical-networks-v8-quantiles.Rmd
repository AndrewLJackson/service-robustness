---
title: "Estimate Robustness of Real Networks - quantile approach"
...
This new approach is based on the median and not the mean!
...
```{r create-a-dataframe-for-jeffs-model, fig.width=15, fig.height=7}
...
g1 <- ggplot(df_webs, aes(x = f_50,
  ...
  #           linetype = 2) + 
  ylab(TeX("$\\textit{R_{50}}$")) + 
  xlab(TeX("$\\textit{f_{50}}$")) +
...
g1_25 <- ggplot(df_webs, aes(x = f_25,
  ...
  # geom_line(data = fit_df, mapping = aes(x = new_f, y = est_r, color = NULL), 
  #           linetype = 2) + 
  ylab(TeX("$\\textit{R_{25}}$")) + 
  xlab(TeX("$\\textit{f_{25}}$")) +
   labs(color = expression(log[10](d))) +
  theme_classic() + 
  theme(axis.text = element_text(size = axis_text_size),
        axis.title = element_text(size = axis_title_size)) + 
...
g1_75 <- ggplot(df_webs, aes(x = f_75,
  ...
  # geom_line(data = fit_df, mapping = aes(x = new_f, y = est_r, color = NULL), 
  #           linetype = 2) + 
  ylab(TeX("$\\textit{R_{75}}$")) + 
  xlab(TeX("$\\textit{f_{75}}$")) +
   labs(color = expression(log[10](d))) +
  theme_classic() + 
  theme(axis.text = element_text(size = axis_text_size),
        axis.title = element_text(size = axis_title_size)) + 
...
# fit breakpoint regression at the origin
mRf_50 <- lm(residRf_50 ~ -1 + I(log10d * (log10d<0)) + I(log10d * (log10d>0)),
           data = df_webs)


mRf_25 <- lm(residRf_25 ~ -1 + I(log10d * (log10d<0)) + I(log10d * (log10d>0)),
           data = df_webs)


mRf_75 <- lm(residRf_75 ~ -1 + I(log10d * (log10d<0)) + I(log10d * (log10d>0)),
           data = df_webs)
...
plot the polynomial test model
```{r}

cubeRoot <- function(x) sign(x) * abs(x) ^ (1/3)

myInvLogit <- function(x){1/(1 + exp(-x))}

erf <- function(x) -(2 * pnorm(x * sqrt(2)) - 1)

# break point around the origin
m_test <- lm(residRf_50 ~ -1 + I(log10d * (log10d<0)) + I(log10d * (log10d>0)),
           data = df_webs)

# m_test <- lm(residRf_50 ~ -1 + I(tanh(log10d)),
#            data = df_webs)

# m_test_neg <- lm(residRf_50 ~ -1 + log10d, 
#            data = df_webs %>% filter(log10d < 0))
# 
# m_test_pos <- lm(residRf_50 ~ -1 + log10d, 
#            data = df_webs %>% filter(log10d > 0))


gtest <- ggplot(df_webs, 
  ...
  geom_line(data = broom::augment(m_test, newdata = df_webs),
  ...
  labs(color = "Web Type", 
       shape = "Web Type") + 
  ylab(TeX("$\\textit{R_{50}} - (1 - \\textit{f_{50}})$")) + 
...
print(gtest)
ggsave(gtest, filename = "images/breakpoint-dispersion.png")
```
...
g3_25 <- ggplot(df_webs, 
  ...
  labs(color = "Web Type", 
       shape = "Web Type") + 
  ylab(TeX("$\\textit{R_{25}} - (1 - \\textit{f_{25}})$")) + 
  xlab(TeX("$\\log_{10}(\\textit{d})$")) +
  theme_classic() + 
  theme(axis.text = element_text(size = axis_text_size),
        axis.title = element_text(size = axis_title_size),
...
g3_75 <- ggplot(df_webs, 
  ...
  labs(color = "Web Type", 
       shape = "Web Type") + 
  ylab(TeX("$\\textit{R_{75}} - (1 - \\textit{f_{75}})$")) + 
  xlab(TeX("$\\log_{10}(\\textit{d})$")) +
  theme_classic() + 
  theme(axis.text = element_text(size = axis_text_size),
        axis.title = element_text(size = axis_title_size),
...
# regress robsustness against (1 - fragility) and then estimate the coefficient
# of the linear regression of the residuals to this plot against
# log10(dispersion) and calculate
# fstar = fragility +  gamma * log10(dispersion)
# df_webs <- df_webs %>% 
#   mutate(fstar = pmax(fragility - .fitted, runif(n(), 0, 0.1)))

# df_webs <- df_webs %>%
#   mutate(fstar_50 = pmax(f_50 - coef(mRf_50) * log10(dispersion), runif(n(), 0, 0.1)))

# df_webs <- df_webs %>%
#   mutate(fstar_50 = f_50 + coef(mRf_50) * log10(dispersion),
#          fstar_25 = f_25 - coef(mRf_25) * log10(dispersion),
#          fstar_75 = f_75 - coef(mRf_75) * log10(dispersion),
#          fstar_50_test = f_50 - 
#            coef(m_test)[1] * log10(dispersion) * (log10(dispersion)<0) - 
#            coef(m_test)[2] * log10(dispersion) * (log10(dispersion)>0)
#          )


df_webs <- df_webs %>% 
  mutate(fstar_50 = f_50 - fitted(mRf_50),
         fstar_25 = f_25 - fitted(mRf_25),
         fstar_75 = f_75 - fitted(mRf_75),
         fstar_50_test = f_50 - 
           coef(m_test)[1] * log10(dispersion) * (log10(dispersion)<0) - 
           coef(m_test)[2] * log10(dispersion) * (log10(dispersion)>0)
  )
...
# plot the data and add a line using broom augment and specify the 
# prediction is to be on the response scale.
g6_test <- ggplot(df_webs, aes(x = fstar_50_test, 
                          y = robustness_50,
                          color = fstar_50_test)) + 
  geom_point(alpha = 0.6) + 
  scale_color_viridis_c(begin = 0, end = 0.0001) +  
  ylab(TeX("$\\textit{R_{50}}$")) + 
  xlab(TeX("$\\textit{f^*_{50}} TEST$")) +
  labs(color = "Web Type", line.style = NULL) +
  theme_classic() + 
  theme(axis.text = element_text(size = axis_text_size),
        axis.title = element_text(size = axis_title_size)) + 
...
print(g6_test | g6)
ggsave((g6_test | g6), filename = "images/compare-f-star-50.png", 
       width = 14, height = 7)
```

**Figure S2.3.** The relationship betwenn Robustness ($R$) and corrected Fragility measure approximated by $f^* = f + \log_{10}(d)$, where for the median (middle panel), $\lambda =$ `r round(abs(coef(mRf_50)),2)`. The 
...
```{r}
df_webs %>% arrange(fstar_50, desc(S), resid_fstar_50, robustness_50) %>% select(file, S, N, resid_fstar_50, robustness_50, dispersion, fstar_50)
```

## Save the raw robustness data

Conver the tibble to a data.frame and export
