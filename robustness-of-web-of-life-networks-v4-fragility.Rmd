...
g1 <- ggplot(df_webs, aes(x = fragility,
  ...
  geom_point() + 
  scale_color_viridis_c(guide = guide_legend(override.aes = list(color = "white"))) +
...
predictRobustnessDagger <- function(x,d) {
  exp(-x - (2/3)* x ^2 ) + (cc[1] + cc[2] * log(d))
    }
...
g6 <-  ggplot(df_webs, aes(x = predictRobustnessDagger(fragility, dispersion),
                     y = robustness,
                     color = log(dispersion))) + 
  geom_point() + 
  scale_color_viridis_c() + 
...
+ $\text{cor}(R,G(f,d))$ = `r round(with(df_webs, cor(robustness, predictRobustnessDagger(fragility, dispersion), method = "spearman")), 2)`
