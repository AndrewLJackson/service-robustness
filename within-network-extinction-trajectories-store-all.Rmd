...
poly_2 <- lme(fixed = zeroed_f_star ~ -1 + I((extinction_sequence-1)^2),
              random =  ~ -1 + I((extinction_sequence-1)^2)|replicate,
              data = res_df)

poly_3 <- lme(fixed = zeroed_f_star ~ -1 + I((extinction_sequence-1)^3),
              random =  ~ -1 + I((extinction_sequence-1)^3)|replicate,
              data = res_df)

poly_4 <- lme(fixed = zeroed_f_star ~ -1 + I((extinction_sequence-1)^4),
              random =  ~ -1 + I((extinction_sequence-1)^4)|replicate,
              data = res_df)


poly_k <- lme(fixed = zeroed_f_star ~ -1 + I((extinction_sequence-1)^3),
              random =  ~ -1 + I((extinction_sequence-1)^3)|replicate,
              data = res_df)


new_extinction <- seq(1, 50, 1)
new_replicates <- 1:reps

new_df <- list()

for (i in new_replicates) {
  
  new_df[[i]] <- data.frame(replicate = i, 
                          extinction_sequence = new_extinction)
  
}

new_df %<>% bind_rows(new_df) 

new_df %<>% mutate(.fitted = predict(poly_k, 
                                     newdata = new_df, 
                                     levels = new_replicates) + offset)
...
# median extinction sequence numbers. We will integrate the polynomials from
# 0 to b
b <- median(res_df$nn)

# use the coefficients to integrate the curves
tmp <- coef(poly_k)
poly_df <- data.frame(replicate = rownames(tmp),
                      beta = tmp[,1]) %>% 
  mutate(area = offset * beta + beta * (b ^ 4) / 4)
